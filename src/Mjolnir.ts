...
export class Mjolnir {
    ...
    private async resyncJoinedRooms(withSync = true): Promise<void> {
        ...
        const oldRoomIdsProtecting = new Set(this.protectedRoomsTracker.getProtectedRooms());
        // find every room that we have left (since last time)
        for (const roomId of oldRoomIdsProtecting.keys()) {
        ...
        for (const roomId of joinedRoomIdsToProtect.keys()) {
            if (!oldRoomIdsProtecting.has(roomId)) {
    ...
    /**
     * loads the watched policy lists from account data, only used when Mjolnir is initialized.
     */
